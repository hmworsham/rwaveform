...
#'@export
process.wf <- function(fp, logpath, outdir){
  ...
  processvector <- function(out.v, re.v, geol.v, outir.v, sysir.v) {
    ...
    try.pm = tryCatch({
      peak.metrics = peak.info(decon, idx, 0.01, step=0.45)
    },
    error=function(e) {
      ParallelLogger::logTrace(
        paste(
    ...
    # If unreasonable peaks, take the largest and resmooth
    if(np>10){
      peak.metrics = peak.metrics[order(peak.metrics$A, decreasing=T),][1:10,]
      decon = runmean(decon[-1], 9, "C")
      decon = c(idx, decon)
      
      # Decompose with a conservative threshold for safety
      decom = safedecomp(decon, thres=0.7)
