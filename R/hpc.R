...
hpc <- function(waveform,geo,thres=0){
 
 idx <- seq(1,nrow(waveform))
  
 waveform[waveform < thres] <- NA
 nr <- nrow(waveform)
 coll <- ncol(waveform)
 ...
 ndat <- matrix(NA,coll*nr,5)
 ndat[,1] <- sort(rep(idx, coll))
 ...
 for (i in 1:coll){
  ...
  ndat[s:e,5] <- intensity
 }
 ndat <- ndat[!is.na(ndat[,5]),]
 colnames(ndat) <- c('index', 'px','py','pz','pi')
 return (ndat)
